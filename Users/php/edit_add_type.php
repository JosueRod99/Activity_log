	...
	<ul>
		...
		&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp
			<table width="25%">
				...
				<?php
  							$i=1;
  							$sql = "SELECT * FROM `type_act` order by TID asc";
  							$result = mysqli_query($conection,$sql);
								while($row=mysqli_fetch_assoc($result)){
									
  						?>
  					<form action="edit_master_type.php?sx=<?php echo $user;?>" method="post">
				<tr>
					<td class="tg-celdas2"> 
						<input type="text" name="UID" value="<?php echo $row['TID']; ?>"  readonly="readonly" > 
					</td>
					<td class="tg-celdas2">
  						<input type="text" name="name" value="<?php if($row['TID'] ==1) echo "OPERACIÓN"; else{echo $row['NAME'];}
  						?>" > <p>
					</td>
